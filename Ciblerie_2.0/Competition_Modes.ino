...
// --- Mode Competition Mutualisé (1 à 4 joueurs) ---

// À inclure dans ton projet, sans redondance ni variable globale dupliquée
void jouerPartieCPT(int nbJoueurs, String mode) {
  Serial.print("Mode : "); Serial.println(mode);
  Serial.println(F("CPT , Ok pour le lancement du jeu"));
  envoyerDebutPartie();
  AcquisitionCapteurs();
  if (initialisation) InitGameCPT(nbJoueurs);
  if (partieEnCours) trtPartieEnCoursCPT(nbJoueurs);
  if (partieFinie) trtPartieFinieCPT(nbJoueurs);
}

// --- Initialisation du jeu ---
void InitGameCPT(int nbJoueurs) {
...
void trtPartieEnCoursCPT(int nbJoueurs) {
  FastLED.clear(); FastLED.show();
  Temporisation();
  EcranEnJeuCPT();
  ...
  while (
    ...
    AcquisitionCapteurs();
    PenaliteCPT(nbJoueurs);
    if (partieFinie) break;
    else killer[joueurEnCours] = 0;
    delay(1);
  }

  gestionCiblesCPT(joueurEnCours);

  SMRAZ();
  GererInterruptionCPT(nbJoueurs);
}

// --- Gestion factorisée des cibles pour Competition (spécifique cible 1 !) ---
void gestionCiblesCPT(int joueur) {
  ...
  for (int i = 0; i < 9; i++) {
    if (digitalRead(cibles[i].pin) == LOW) {
      ...
      // Message ESP32 (optionnel, comme dans MS/CF, sinon retire ces lignes)
...
// --- Pénalité Competition (temps, mort subite générale en cas d'échec) ---
void PenaliteCPT(int nbJoueurs) {
...
// --- Fin de partie ---
void trtPartieFinieCPT(int nbJoueurs) {
  FinGame();
...
void GererInterruptionCPT(int nbJoueurs) {

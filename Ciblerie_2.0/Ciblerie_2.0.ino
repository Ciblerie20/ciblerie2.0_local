...
// Ok le 06.06.25 à 9h - Code 100% fonctionnel
...
// ============================
// DEBUT DU PROGRAMME VARIABLES
// ============================

// Variable globale pour le mode courant
//String modeCourant = "CL"; // ou "CF", "LJ", "MS", "CPT", etc.
int nbJoueursActifs = 1; // 1 à 4
// Définitions des modes (toujours AVANT de les utiliser)
#define MODE_CF   1
#define MODE_CSP  2
#define MODE_CP   3
#define MODE_CC   4
...
#define MODE_CPT  8
// Variable globale pour le mode courant
int modeCourant = MODE_CF; // MODE initial
...
void loop() 
{
  ...
  communications();
    ...
    switch (modeCourant) {
      case MODE_CF:  jouerPartieCF(nbJoueursActifs, "CF");    break;
      case MODE_CSP: jouerPartieCSP(nbJoueursActifs, "CSP");  break;
      case MODE_CP:  jouerPartieCP(nbJoueursActifs, "CP");    break;
      case MODE_CC:  jouerPartieCC(nbJoueursActifs, "CC");    break;
      case MODE_CL:  jouerPartieCL(nbJoueursActifs, "CL");    break;
      case MODE_LJ:  jouerPartieLJ(nbJoueursActifs, "LJ");    break;
      case MODE_MS:  jouerPartieMS(nbJoueursActifs, "MS");    break;
      case MODE_CPT: jouerPartieCPT(nbJoueursActifs, "CPT");  break;
    }
...
// --- STRUCTURE CORRIGÉE ---
struct CibleConfig {
  bool* statusPtr; // Corrigé pour bool
  int pin;
  int basePoints;
  int* pointBonusArray;
  void (*resetAllBonus)();
  CRGB couleur;
  int mp3FolderIdx;
  void (*bonusFn)(); // Peut être nullptr
};

// --- EXEMPLE DE VARIABLES (adapte les noms si besoin) ---
bool statusCible1 = false;
bool statusCible2 = false;
bool statusCible3 = false;
bool statusCible4 = false;
bool statusCible5 = false;
bool statusCible6 = false;
bool statusCible7 = false;
bool statusCible8 = false;
bool statusCible9 = false;
bool statusCible10 = false;

// Points bonus par joueur (exemple, à adapter selon ta déclaration globale)
int pointBonus0[5], pointBonus5[5], pointBonus10[5], pointBonus15[5], pointBonus25[5];
int pointBonus50[5], pointBonus100[5], pointBonus150[5], pointBonus200[5], pointBonus250[5];

// --- EXEMPLE DE RESET (adapte à tes fonctions existantes) ---
void resetAllBonus0() {}
void resetAllBonus5() {}
...
void CentCinquante() {}
void DeuxCent() {}
void DeuxCentCinquante() {}

// --- TABLEAU CIBLES CORRIGÉ ---
CibleConfig cibles[10] = {
  { &statusCible1, 22,   0,   pointBonus0,   resetAllBonus0,     CRGB::Red,         1, nullptr },
  { &statusCible2, 24,   5,   pointBonus5,   resetAllBonus5,     CRGB::Orange,      2, nullptr },
  { &statusCible3, 26,  10,   pointBonus10,  resetAllBonus10,    CRGB::Yellow,      3, nullptr },
  { &statusCible4, 28,  15,   pointBonus15,  resetAllBonus15,    CRGB::Green,       5, nullptr },
  { &statusCible5, 30,  25,   pointBonus25,  resetAllBonus25,    CRGB::Blue,        8, nullptr },
  { &statusCible6, 32,  50,   pointBonus50,  resetAllBonus50,    CRGB::White,      10, Cinquante },
  { &statusCible7, 34, 100,   pointBonus100, resetAllBonus100,   CRGB::Purple,     12, Cent },
  { &statusCible8, 36, 150,   pointBonus150, resetAllBonus150,   CRGB::Aqua,       14, CentCinquante },
  { &statusCible9, 38, 200,   pointBonus200, resetAllBonus200,   CRGB::GreenYellow,16, DeuxCent },
  { &statusCible10,40,250,   pointBonus250, resetAllBonus250,   CRGB::BlueViolet, 18, DeuxCentCinquante }
};

// --- FONCTION DE GESTION DES CIBLES ---
void gestionCibles(int joueurEnCours) {
  for (int i = 0; i < 10; ++i) {
    *(cibles[i].statusPtr) = digitalRead(cibles[i].pin);
    if (*(cibles[i].statusPtr) == LOW) {
      ...
      int bonusValBefore = cibles[i].pointBonusArray[joueurEnCours];
      if (i == 0) {
        scores[joueurEnCours] += 0;
      } else {
        scores[joueurEnCours] += cibles[i].basePoints + cibles[i].pointBonusArray[joueurEnCours];
        int bonusInc = 0;
        switch (i) {
        ...
        cibles[i].pointBonusArray[joueurEnCours] += bonusInc;
      }

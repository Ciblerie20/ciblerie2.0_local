...
// 📨 📥
// Fonction d'envoi de message à l'ESP32
...
// Fonction principale de gestion des communications entrantes
void communications() {
  if (Serial1.available() > 0) {
    ...
    if (
      ...
      && (
        ...
        || strcmp(messageRecu, "CPT1") == 0 || strcmp(messageRecu, "CPT2") == 0 || strcmp(messageRecu, "CPT3") == 0 || strcmp(messageRecu, "CPT4") == 0
      )
    ) {
      if (partieDemarree) {
        Serial.print("⚠️ Message de groupe ignoré car partie déjà démarrée : ");
        Serial.println(messageRecu);
        return; // On ignore ce message si la partie est déjà démarrée
      }
    ...
    if (strcmp(messageRecu, "START_GAME") == 0 && !partieDemarree) {
      ...
      else if (groupeSelectionne == "CPT2") { CPT2(); }
      else if (groupeSelectionne == "CPT3") { CPT3(); }
      else if (groupeSelectionne == "CPT4") { CPT4(); }
      else {
    ...
    if (
      (strcmp(messageRecu, "NEXT_PLAYER") == 0 || strcmp(messageRecu, "NEXT_TURN") == 0)
      && partieDemarree
    ) {
      Serial.print("📥 Message reçu de l'ESP32 : ");
      Serial.println(messageRecu);
    ...
    // 5. Fin de partie
    if (strcmp(messageRecu, "FIN_GAME") == 0 && partieDemarree) {

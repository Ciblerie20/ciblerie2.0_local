...
class _ScoresScreenState extends State<ScoresScreen> with TickerProviderStateMixin {
  ...
  // ----- CUMUL DU TEMPS DE JEU (utilise la value du WebSocketService) -----
  // PAS de gestion locale ici, tout passe par ws.cumulativeTimeNotifier !
  // ----------------------------------
    ...
    setState(() {
      if (type == 'game_status') {
        ...
        if (wsMessage == 'FIN_GAME') {
          ...
          // Remise à zéro du cumul à la fin du jeu (décommente la ligne si tu veux reset après chaque partie)
          // ws.cumulativeTimeNotifier.value = 0.0;
        }
      }
    });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          // Affiche le temps cumulé du joueur courant
          ValueListenableBuilder<Map<int, double>>(
            valueListenable: ws.cumulativeTimesNotifier,
            builder: (context, cumulTimes, _) {
              final cumul = cumulTimes[uiState.currentPlayerIndex] ?? 0.0;

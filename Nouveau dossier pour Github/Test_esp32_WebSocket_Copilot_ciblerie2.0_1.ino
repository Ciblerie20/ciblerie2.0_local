...
int scores[4] = {0, 0, 0, 0};    // Scores des joueurs (J1, J2, J3, J4)
...
void processIncomingSerialData() {
  while (Serial1.available()) {
    ...
    if (incomingData.startsWith("J")) { // Vérifie si le format est correct pour un joueur
      ...
      int score = incomingData.substring(4).toInt(); // Extrait le score

      if (playerIndex >= 0 && playerIndex < 4) {
        scores[playerIndex] = score; // Met à jour le score
        Serial.printf("📥 Reçu : %s\n", incomingData.c_str()); // Affiche le message reçu

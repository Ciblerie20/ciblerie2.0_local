          ...
          if (isConnected)
            Padding(
              ...
              child: ElevatedButton.icon(
                ...
                onPressed: () {
                  // Vérification de connexion au WebSocket avant d'envoyer la commande
                  if (webSocketService.isConnected) {
                    // Réinitialiser les scores localement
                    webSocketService.resetScores();
                    // Envoyer la commande de réinitialisation au serveur
                    webSocketService.sendResetCommand();
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('🆕 Nouvelle partie lancée')),
                    );
                  } else {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('❌ Impossible de lancer une nouvelle partie : non connecté.')),
                    );
                  }
                },
